    ...
    def get_all_memes(self):
        ...
        for file in os.listdir(self.memes_dir):
            if file.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.mp4', '.mov')):
                memes.append({'filename': file, 'path': os.path.join(self.memes_dir, file)})
        return memes
    ...
    def get_all_situations(self):
        if not os.path.exists(self.situations_file):
            sample_situations = [
                ...
                "Когда забыл выключить утюг и уже едешь в метro",
                "Когда пытаешься работать после обеда"
            ]
